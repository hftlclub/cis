...
clubAdminApp.controller('protocolFormController', function($scope, $http, $routeParams, $route, $interval, clubAuth) {
  var form = {};
  $scope.users = [];
  $scope.form = form;
...
$scope.commonTitles = ['Clubsitzung', 'Mitgliederversammlung', 'Planungstreffen']; form.id = $routeParams.id; form.mode = $route.current.locals.clubMode;

form.errors = {}; form.message = null;
...
$scope.form.protocolData.date = new Date(); $scope.minDate = $scope.minDate ? null : new Date(2012, (10 - 1), 25); $scope.maxDate = $scope.maxDate ? null : new Date();
  ...
  if (form.mode == 'edit' && form.id) {
    ...
    $http.post(apiPath + '/protocols', form.protocolData)
      .success(function(data) {
        ...
        //set id and form mode to 'edit' to save automatically
        form.id = data.id;
        form.mode = 'edit';
      })
      .error(function(data, status) {
